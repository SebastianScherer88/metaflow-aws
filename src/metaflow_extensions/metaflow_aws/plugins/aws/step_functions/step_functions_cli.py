...
from .step_functions_client import CustomStepFunctionsState
...
def create(
    ...
    log_execution_history: bool = False,
    use_distributed_map: bool = False,
    compress_state_machine: bool = False,
    ...
    if only_json:
        ...
        flow.create(log_execution_history)
        obj.echo(
...
def resolve_token(
    ...
    workflow = CustomStepFunctions.get_existing_deployment(name)
    if workflow is None:
        obj.echo(
...
def list_runs(
    ...
    states = []
    if running:
        states.append(CustomStepFunctionsState.running)
    if succeeded:
        states.append(CustomStepFunctionsState.succeeded)
    if failed:
        states.append(CustomStepFunctionsState.failed)
    if timed_out:
...
def validate_run_id(
    ...
    name = run_id[4:]
    execution = CustomStepFunctions.get_execution(state_machine_name, name)
